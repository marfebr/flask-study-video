...
from wfdb.models import Actor, db
from .parsers import actor_post_parser, actor_put_parser, token_parser
from .auth import abort_if_no_admin_auth
nested_role_fields = {
    'movie_id': fields.Integer(),
    'role_mame': fields.String()
}
actor_fields = {
    'id': fields.Integer(),
...
class ActorAPI(Resource):
    ...
    def post(self, actor_id=None):
        if actor_id:
            ...
            new_actor.birthday = datetime.datetime.strptime(args['birthday'], '%Y-%m-%d').date()

            if args['deathday']:
                new_actor.deathday = args['deathday']
    ...
    def put(self, actor_id=None):
        if not actor_id:
            ...
            if args['birthday']:
               actor.birthday =datetime.datetime.strptime(args['birthday'], '%Y-%m-%d').date()
            
            if args['deathday']:
                actor.deathday = args['deathday']
