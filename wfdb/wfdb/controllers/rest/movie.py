...
from wfdb.models import Movie, db, Actor
from .parsers import movie_post_parser, movie_put_parser, token_parser
...
class MovieAPI(Resource):
    ...
    def post(self, movie_id=None):
        if movie_id:
            ...
            new_movie.release_date = datetime.datetime.strptime(args['release_date'],'%Y-%m-%d').date()

            director_id = args['director_id']
            director = Actor.query.get(director_id)

            if director is None:
                abort(400, message='Director with id {} does not exists'.format(director_id))
    ...
    def put(self, movie_id=None):
        if not movie_id:
            ...
            if args['release_date']:
                movie.release_date = datetime.datetime.strftime(
                    args['release_date'],
                    '%Y-%m-%d'
                    ).date()
            if args['director_id']:
